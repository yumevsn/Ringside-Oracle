...
// Action to fetch CSV data (actions can use fetch)
export const seedInitialData = action({
  args: {},
  handler: async (ctx) => {
    // Check if data already exists
    const existingPromotions = await ctx.runQuery(internal.seedData.checkExistingData)
    ...
    try {
      ...
      console.log("CSV files fetched, importing to database...")

      // Call the mutation to insert data
      const result = await ctx.runMutation(internal.seedData.insertCSVData, {
        promotionsCSV,
        brandsCSV,
        wrestlersCSV,
        eventsCSV,
...
// Internal mutation to insert CSV data
export const insertCSVData = mutation({
  args: {
    promotionsCSV: v.string(),
    brandsCSV: v.string(),
    wrestlersCSV: v.string(),
    eventsCSV: v.string(),
    matchTypesCSV: v.string(),
    championshipsCSV: v.string(),
  },
  handler: async (ctx, args) => {
    // Parse CSV data
    const promotionsData = parseCSV(args.promotionsCSV)
    const brandsData = parseCSV(args.brandsCSV)
    const wrestlersData = parseCSV(args.wrestlersCSV)
    const eventsData = parseCSV(args.eventsCSV)
    const matchTypesData = parseCSV(args.matchTypesCSV)
    const championshipsData = parseCSV(args.championshipsCSV)
